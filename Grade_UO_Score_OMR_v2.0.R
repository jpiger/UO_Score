# Grade Exams Based on Output File from UO_Score_OMR_v2.0.R
# Jeremy Piger
# version 1.0
...
for (i in 1:dim(Student_Answers)[1]) {
  ...
  if (num_versions>1) {
    ...
    if (is.na(form_i)==FALSE && form_i>=1 && form_i<=num_versions) {
      key_form_i = as.data.frame(Answer_Key[,form_i])
    }
    
    if (is.na(form_i) || form_i<1 || form_i>num_versions) {
      ...
      print_name <- paste0(last_name_i,", ",first_name_i,", ",sid_i)
      print_update1 = paste0("No version or invalid version selected by ", print_name, " (page number ", page_number_i, ").")
